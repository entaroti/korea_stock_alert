...
def get_uptrend_stocks(market: str = "ALL", top_n: int = 30) -> pd.DataFrame:
    """
    최근 1주(5거래일) 우상향 종목 리스트를 반환합니다.

    [우상향 조건]
      1) 최근 5거래일 수익률 > 0
      2) 5일 이동평균 > 10일 이동평균
      3) 5일 이동평균이 5거래일 전보다 상승 (최근 1주 동안 우상향)

    [반환 컬럼]
      code, name, ret_5d, avg_tv_10d, avg_tv_5d
    """
    ...
    for code in tickers:
        try:
            ...
            # 최소 15거래일 이상 데이터 필요 (5일 수익률 + 5/10일선, 5일 전 비교)
            if df.shape[0] < 15:
                continue
            ...
            # 최근 5거래일 기준 (1주 수익률)
            last_5_close = closes.tail(5)
            last_10_tv = trading_value.tail(10)  # 10일 평균용
            last_5_tv = trading_value.tail(5)    # 5일 평균용

            # (1) 5거래일 수익률
            ret_5d = last_5_close.iloc[-1] / last_5_close.iloc[0] - 1
            ...
            # 5거래일 전 5일선 (최근 1주 동안 기울기 확인)
            if len(ma5) < 6:
                continue
            ma5_5days_ago = ma5.iloc[-6]

            # 우상향 조건
            cond_price_up = ret_5d > 0               # 최근 1주 수익률 플러스
            cond_ma_position = ma5_last > ma10_last  # 5일선 > 10일선
            cond_ma_slope = ma5_last > ma5_5days_ago # 최근 1주 동안 5일선 우상향
    ...
    # 5거래일 수익률 기준 내림차순 정렬
    df_res = df_res.sort_values(by="ret_5d", ascending=False).reset_index(drop=True)
...
def main():
    ...
    if df_up.empty:
        send_message("📭 최근 1주(5거래일) 우상향 조건을 만족하는 종목이 없습니다.")
        return
    ...
    # 2열: 종목명
    # 3열: 최근 5거래일 수익률
    # 4열: 최근 10영업일 일평균 거래대금
    # 5열: 최근 5영업일 일평균 거래대금
    header = "번호\t종목명\t수익률(5일)\t10일평균거래대금(억)\t5일평균거래대금(억)"
    table_lines = [header]

    for i, row in df_up.iterrows():
        rank = i + 1
        name = row["name"]
        ret_5d_pct = row["ret_5d"] * 100.0
    ...
    message = (
        "📈 *최근 1주(5거래일) 우상향 종목 Top 30*\n"
        "_(조건: 5거래일 수익률>0, 5일선>10일선, 5일선 1주 우상향)_\n\n"
        "```text\n"
        f"{table_text}\n"
        "```"
    )
